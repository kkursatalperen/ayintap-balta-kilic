...
export async function sendPasswordResetEmail({ to, link }) {
  ...
  return sendEmail({
    ...
    text: `Şifre sıfırlama bağlantınız: ${link}`,
  });
}
...
export async function sendWelcomeEmail({ to, name }) {
  ...
  const base = process.env.NEXT_PUBLIC_BASE_URL || '';
  const body = `
    <p style="margin:0 0 12px 0;">Merhaba <strong style="color:${BRAND.gold};">${escapeHtml(name || 'Değerli Üyemiz')}</strong>,</p>
    <p style="margin:0 0 16px 0;">Ayıntap Kılıç ailesine hoşgeldiniz! Geleneksel Türk çelik sanatını yaşatan koleksiyonumuzu keşfetmek için aşağıdaki butona tıklayın.</p>
    <p style="margin:0 0 16px 0;color:${BRAND.muted};font-size:13px;">Herhangi bir sorunuz için <a href="mailto:info@ayintapkilic.com" style="color:${BRAND.gold};">info@ayintapkilic.com</a> adresinden bize ulaşabilirsiniz.</p>
  `;
  return sendEmail({
    to,
    subject: `Hosgeldiniz · ${BRAND.name}`,
    html: baseLayout({
      title: 'Ailemize Hoşgeldiniz',
      preheader: 'Ayıntap Kılıç ailesine katıldınız!',
      bodyHtml: body,
      ctaText: 'Koleksiyonu Keşfet',
      ctaUrl: base ? `${base}/urunler` : null,
    }),
    text: `Hosgeldiniz ${name || ''}! Ayıntap Kılıç ailesine katıldınız.`,
  });
}
// test