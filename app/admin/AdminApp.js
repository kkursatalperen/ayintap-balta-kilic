...
function UsersPanel() {
  ...
  const exportCSV = () => {
    const headers = ['İsim', 'E-posta', 'Telefon', 'Kayıt Tarihi', 'Sipariş Sayısı', 'Toplam Harcama', 'Durum'];
    ...
    a.href = url;
    a.download = `uyeler_${new Date().toLocaleDateString('tr-TR').replace(/\./g, '-')}.csv`;
    a.click();
    ...
    <div className="p-10">
      ...
      {selectedIds.length > 0 && (
        <div className="flex items-center gap-3 mb-4 p-3 bg-amber-500/10 border border-amber-500/30 rounded-lg">
          <span className="text-amber-400 text-sm font-serif">{selectedIds.length} üye seçildi</span>
          <select value={bulkAction} onChange={(e) => setBulkAction(e.target.value)} className="bg-black/40 border border-amber-500/30 rounded px-3 py-1.5 text-amber-50 text-sm focus:outline-none">
            <option value="">İşlem Seç</option>
            <option value="block">Engelle</option>
            <option value="unblock">Engeli Kaldır</option>
          </select>
          <button onClick={async () => {
            if (!bulkAction) return;
            if (!confirm(`${selectedIds.length} üyeye "${bulkAction === 'block' ? 'Engelle' : 'Engeli Kaldır'}" işlemi uygulanacak. Onaylıyor musunuz?`)) return;
            await Promise.all(selectedIds.map(id =>
              fetch(`/api/admin/users/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ isBlocked: bulkAction === 'block' }) })
            ));
            setSelectedIds([]);
            setBulkAction('');
            load();
          }} className="bg-amber-500 text-black font-bold px-3 py-1.5 rounded text-sm font-serif">UYGULA</button>
          <button onClick={() => setSelectedIds([])} className="text-amber-100/50 text-sm hover:text-amber-100">Seçimi Temizle</button>
        </div>
      )}
      ...
      <div className="flex gap-3 mb-6">
        ...
        <button onClick={exportCSV} className="border border-amber-500/30 text-amber-400 font-bold px-4 py-2.5 rounded font-serif tracking-widest text-sm hover:bg-amber-500/10 transition flex items-center gap-2">
          <Save size={14}/> CSV İNDİR
        </button>
      </div>
        ...
        <div className="bg-[#161616] border border-amber-500/10 rounded-lg overflow-x-auto">
          <table className="w-full min-w-[700px]">
            <thead className="bg-black/30 border-b border-amber-500/10">
              <tr className="text-left text-xs text-amber-400 font-serif tracking-widest">
                <th className="p-4">
                  <input type="checkbox" className="accent-amber-500" checked={selectedIds.length === users.length && users.length > 0} onChange={(e) => setSelectedIds(e.target.checked ? users.map(u => u.id) : [])}/>
                </th>
            ...
            <tbody>
              ...
              {users.map((u) => (
                <tr key={u.id} className="border-b border-amber-500/5 hover:bg-amber-500/5">
                  <td className="p-4">
                    <input type="checkbox" className="accent-amber-500" checked={selectedIds.includes(u.id)} onChange={(e) => setSelectedIds(e.target.checked ? [...selectedIds, u.id] : selectedIds.filter(id => id !== u.id))}/>
                  </td>
                  <td className="p-4">
...
function CouponsPanel() {
  ...
  const save = async () => {
    ...
    const d = await res.json();
    if (res.ok) { toast.success('Kupon oluşturuldu'); setForm({ code: '', discount: 10, type: 'percent', minOrder: 0, maxUses: 0, expiresAt: '' }); load(); }
  ...
  return (
    <div className="p-10">
      <h1 className="font-serif text-4xl text-amber-50 mb-2">Kupon Yönetimi</h1>
      <p className="text-amber-100/50 mb-8">İndirim kuponları oluştur ve yönet</p>

      {/* Yeni Kupon Formu */}
      <div className="bg-[#161616] border border-amber-500/10 rounded-lg p-6 mb-8">
        <h2 className="font-serif text-xl text-amber-400 mb-5">Yeni Kupon Oluştur</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <div>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={generateCode} className="px-3 border border-amber-500/30 rounded text-amber-400 hover:bg-amber-500/10 text-xs">ÜRETt</button>
            </div>
          </div>
          <div>
            <label className="text-xs text-amber-400 font-serif tracking-widest block mb-1">İNDİRİM TÜRÜ</label>
          ...
          <div>
            <label className="text-xs text-amber-400 font-serif tracking-widest block mb-1">{form.type === 'percent' ? 'İNDİRİM (%)' : 'İNDİRİM (₺)'}</label>
          ...
          <div>
            <label className="text-xs text-amber-400 font-serif tracking-widest block mb-1">MİNİMUM SİPARİŞ (₺)</label>
          ...
          <div>
            <label className="text-xs text-amber-400 font-serif tracking-widest block mb-1">MAX KULLANIM (0 = Sınırsız)</label>
          ...
          <div>
            <label className="text-xs text-amber-400 font-serif tracking-widest block mb-1">SON KULLANIM TARİHİ</label>
        ...
        <button onClick={save} disabled={saving} className="mt-5 bg-gradient-to-r from-amber-500 to-amber-600 text-black font-bold px-6 py-3 rounded font-serif tracking-widest flex items-center gap-2 hover:from-amber-400 hover:to-amber-500 transition">
          <Plus size={16}/> {saving ? 'OLUŞTURULUYOR...' : 'KUPON OLUŞTUR'}
        </button>
      </div>
        ...
        <table className="w-full min-w-[600px]">
          <thead className="bg-black/30 border-b border-amber-500/10">
            <tr className="text-left text-xs text-amber-400 font-serif tracking-widest">
              ...
              <th className="p-4">İNDİRİM</th>
              <th className="p-4">MİN. SİPARİŞ</th>
              <th className="p-4">KULLANIM</th>
              <th className="p-4">SON TARİH</th>
              <th className="p-4">DURUM</th>
              <th className="p-4">İŞLEM</th>
            </tr>
          </thead>
          <tbody>
            {coupons.length === 0 && <tr><td colSpan={7} className="p-10 text-center text-amber-100/50">Henüz kupon yok.</td></tr>}
            {coupons.map((c) => (
              <tr key={c.id} className="border-b border-amber-500/5 hover:bg-amber-500/5">
                ...
                <td className="p-4 text-amber-100">{c.usedCount}/{c.maxUses > 0 ? c.maxUses : '∞'}</td>
                <td className="p-4 text-amber-100/60 text-xs">{c.expiresAt ? new Date(c.expiresAt).toLocaleDateString('tr-TR') : 'Süresiz'}</td>
                <td className="p-4">
